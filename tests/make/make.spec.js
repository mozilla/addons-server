...
test('map docker compose config', () => {
  ...
  const { stdout: rawConfigDev } = spawnSync(
    'docker',
    ['compose', 'config', '--format', 'json'],
    {
      encoding: 'utf-8',
      env: { ...process.env, ...{ COMPOSE_FILE: 'docker-compose.yml:docker-compose.dev.yml' } },
    },
  );

  const configDev = JSON.parse(rawConfigDev);
  const { web: webDev } = configDev.services;
});
