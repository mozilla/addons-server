...
function checkTimeout(validation) {
  let timeout_id = ['validator', 'unexpected_exception', 'validation_timeout'];
...
$.fn.addonUploader = function (options) {
  ...
  return $(this).each(function () {
    ...
    $upload_field.on('upload_errors', function (e, file, errors, results) {
      let all_errors = $.extend([], errors); // be nice to other handlers
      ...
      let errors_ul = $('<ul>', { id: 'upload_errors' });
    ...
    const compatibilityCheckboxes = $('.compatible-apps input[type=checkbox]');
    let $newForm = $('.new-addon-file');
    let $channelChoice = $('input[name="channel"]');
    ...
    // parameter.
    let $submitAddonProgress = $('.submit-addon-progress');
    ...
    $upload_field.on('upload_success_results', function (e, file, results) {
      if (results.error) {
        ...
        let error = gettext('Unexpected server error while validating.');
      ...
      if (!results.validation) {
        ...
        setTimeout(function () {
          $.ajax({
            ...
            error: function (xhr) {
              let errOb = parseErrorsFromJson(xhr.responseText, xhr.status);
        ...
        if (results.validation.messages) {
          for (let i = 0; i < results.validation.messages.length; i++) {
            ...
            let matched = _.find(checklistWarningsIds, function (warningId) {
              return matchId(warningId, current);
            });
            if (matched) {
            ...
            let foundMv3 = _.find([mv3NoticeId], function (noticeId) {
              return matchId(noticeId, current);
            });
            if (foundMv3) {
