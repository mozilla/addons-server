...
if (typeof SyntaxHighlighter == 'undefined')
  var SyntaxHighlighter = (function () {
    ...
    sh.Highlighter.prototype = {
       ...
       * @return {Array}              Returns list of matches.
       */
      findMatchesNew: function (regexList, code) {
        // Rev:20110216_1700
        ...
        function defaultAdd(m, r) {
          return m[0];
        } // Dummy brush modify result function.
        var nextMatch = true; // Prime the pump.
        while (nextMatch) {
          // One regex match per loop. Advance pos through code.
          ...
          for (var i = 0; i < nregex; i++) {
            // Loop though all regexes at this pos.
            if (typeof (re = regexList[i]) === 'object' && re.regex.global) {
              // Process only 'g'-flagged regexes.
              re.regex.lastIndex = pos; // Start search at same pos for each regex.
              if ((match = re.regex.exec(code)) !== null) {
                // Check if this regex matches anywhere?
                if (match.index === pos) {
                  // Yes. Check if matched on first char?
                  nextMatch = {
                    r: re,
                    m: match,
                  }; // Yes. This is unconditionally our next match.
                ...
                if (match.index < minIndex) {
                  ...
                  nextMatch = {
                    r: re,
                    m: match,
                  }; // Save needed items for Match object.
                }
              }
            } // End if re is object.
          } // End for loop.
          if (nextMatch) {
            // Check if one of the regexes matched?
            ...
            if (typeof resultMatch === 'string')
              // Check if we need a new Match object?
              resultMatch = new sh.Match(
                resultMatch,
                nextMatch.m.index,
                nextMatch.r.css,
              ); // Yes.
            // Check if the result is an html inline script, in which case it's wrapped in another array.
            if (Array.isArray(resultMatch)) {
              matches = matches.concat(resultMatch); // Unwrap and add it.
            } else {
