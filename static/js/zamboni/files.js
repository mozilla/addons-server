...
var Highlighter = {
  ...
  diff: function diff(left, right, brush) {
    ...
    var charred_inputs = differ.diff_linesToChars_(
      this.squash_space(right),
      this.squash_space(left),
    );

    // Then have the diff library diff thos.
    var diffs = differ.diff_main(charred_inputs[0], charred_inputs[1], false);
  ...
  highlight_lines: function highlight_lines(text, brush) {
    ...
    var $node = $('<pre>', {
      class: format('brush: {0}; toolbar: false;', [brush]),
      text: text,
    });
    $('<div>').append($node);
  ...
  highlight: function highlight($node) {
    ...
    if ($node.is('[data-content]')) {
      ...
      var lines = _.map(this.highlight_lines(content, brush), function (
        line,
        idx,
      ) {
        return {
    ...
    $node.html(html);
    $node
      .find('.highlighter-column-line-numbers')
      .css('width', lines_width + 'ex');
  },
};
...
if (typeof SyntaxHighlighter !== 'undefined') {
  ...
  SyntaxHighlighter.Highlighter.prototype.getLineHtml = function (
    lineIndex,
    lineNumber,
    code,
  ) {
...
jQuery.fn.numberInput = function (increment) {
  this.each(function () {
    ...
    var $dom = $('<span>', { class: 'number-combo' })
      .append(
        $('<a>', { class: 'number-combo-button-down', href: '#', text: '↓' }),
      )
      .append(
        $('<a>', { class: 'number-combo-button-up', href: '#', text: '↑' }),
      );

    var $up = $dom.find('.number-combo-button-up').click(
      _pd(function (event, count) {
        count = count || (event.ctrlKey ? increment : 1) || 1;
        $self.val(Number($self.val()) + count);
        $self.change();
      }),
    );
    var $down = $dom.find('.number-combo-button-down').click(
      _pd(function (event, count) {
        count = count || (event.ctrlKey ? increment : 1) || 1;
...
jQuery.fn.appendMessage = function (message) {
  $(this).each(function () {
    ...
    if (!$container.length) {
      ...
      $self
        .append($('<div>', { class: 'message' }).append($container))
        .addClass('message-container');
    }

    $container.append(
      $('<div>')[typeof message == 'string' ? 'text' : 'html'](message),
    );
  });
  return this;
};
...
function bind_viewer(nodes) {
  ...
  function Viewer() {
    ...
    this.message_classes = function (message, base_class) {
      ...
      return classes.map(function (cls) {
        return [base_class, message.type, cls].join('');
      });
    };
      ...
      _.each(
        ...
        function (message) {
          ...
          if (message.line != null && $line.length) {
            $line
              .addClass(this.message_classes(message).join(' '))
              .parent()
      ...
      if ($diff.length || messages) {
        ...
        if ($lines.length) {
          ...
          var flush = function ($line, bottom) {
            var top =
              (($start.offset().top - gutter_top) * 100) / gutter_height;
            var height = ((bottom - $start.offset().top) * 100) / gutter_height,
              style = { height: Math.min(height, 100) + '%', top: top + '%' };
          ...
          $.each(changes, function (i, change) {
            ...
            var $link = $('<a>', {
              href: $start.attr('href'),
              class: $start.attr('class'),
              css: style,
            }).appendTo($sb);

            if ($start.is('.error, .notice, .warning')) {
              $link.appendMessage(
                $start.parent().find('.message-inner > div').clone(),
              );
            }
          });
      ...
      if (data.validation) {
        ...
        this.nodes.$files.find('.file').each(function () {
          ...
          if (known) {
            var msg = ['Identified:'];
            if ('library' in known) {
              msg.push(format('    Library: {library} {version}\n', known));
            }
      ...
      if (data.error) {
        $('#validating').after(
          $('<div>', {
            ...
            text: format('{0} {1}', file_metadata.validationFailed, data.error),
          }),
        );
      }
    };
    this.fix_vertically = function ($node, changes, resize) {
      ...
      if (resize) {
        changes.push([
          $node,
          { height: Math.min(rect.bottom - rect.top, window_height) + 'px' },
        ]);
      }
    ...
    this.update_viewport = function (resize) {
      ...
      if ($viewport) {
        var $gutter = this.$gutter,
          ...
          height = Math.max(
            0,
            Math.min(gr.bottom, $(window).height()) - Math.max(gr.top, 0),
          );
    ...
    this.show_selected = function () {
      ...
      if ($sel.position().top < 0) {
        ...
        var offset =
          $sel.position().top + $sel.outerHeight() - $cont[0].clientHeight;
        if (offset > 0) {
    ...
    this.load = function ($link) {
      ...
      $old_wrapper.load(
        $link.attr('href').replace('/file/', '/fragment/') +
          ' #content-wrapper',
    ...
    this.toggle_files = function (action) {
      var collapse = null;
      if (action == 'hide') collapse = true;
      else if (action == 'show') collapse = false;

      $('#file-viewer').toggleClass('collapsed-files', collapse);
    };
    this.toggle_known = function (hide) {
      if (hide == null) hide = storage.get('files/hide-known');
      else storage.set('files/hide-known', hide ? 'true' : '');
      ...
      if (forward) {
        ...
        for (var i = 0; i < $deltas.length; i++) {
          ...
          if (span.getBoundingClientRect().bottom > height) {
            span =
              $lines[Math.max(0, $lines.indexOf(span) - config.diff_context)];
        ...
        if (!res) {
          ...
          res =
            $lines[
              Math.min(
                $lines.length - 1,
                $lines.indexOf(res) + config.diff_context,
              )
            ];
  ...
  viewer.nodes.$files.find('.directory').click(
    _pd(function () {
  ...
  $(window)
    .resize(
      _.throttle(function () {
        viewer.update_viewport(true);
      }, 10),
    )
    .scroll(
      _.throttle(function () {
        viewer.update_viewport(false);
      }, 10),
    );

  $('#toggle-known').change(function () {
    viewer.toggle_known(this.checked);
  });
  viewer.toggle_known();
  ...
  $('#files-change-prev').click(
    _pd(function () {
      viewer.next_delta(false);
    }),
  );

  $('#files-change-next').click(
    _pd(function () {
  ...
  if (file_metadata.validation) {
    viewer.update_validation(
      { validation: file_metadata.validation, error: null },
      true,
    );
    ...
    $.ajax({
      ...
      success: function (data) {
        if (typeof data != 'object') data = { error: data };
  ...
  viewer.nodes.$files.find('.file').click(
    _pd(function () {
      viewer.select($(this));
  ...
  $(document).on('keypress', function (e) {
    if (
      ...
      ![HTMLInputElement, HTMLSelectElement, HTMLTextAreaElement].some(
        function (iface) {
          return e.target instanceof iface;
        },
      )
    ) {
  ...
  var $tabstops = $('#tab-stops')
    ...
    .change(function (event, global) {
      ...
      if (!global) storage.set(localTabstopsKey, $(this).val());
...
$(document).ready(function () {
  ...
  function poll_file_extraction() {
    $.getJSON($('#extracting').attr('data-url'), function (json) {
      if (json && json.status) {
        $('#file-viewer').load(
          window.location.pathname + '?full=yes' + ' #file-viewer',
          function () {
            $(this).children().unwrap();
  ...
  $right
    .change(function (event) {
      var right = $right.val();
      $left_options.prop('disabled', function () {
  ...
  $left
    .change(function (event) {
      var left = $left.val();
      $right_options.prop('disabled', function () {
