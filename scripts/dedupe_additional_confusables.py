...
for letter in string.ascii_letters[:26][::-1]:
    ...
    for character in additional_character_replacements[letter]:
        ...
        variants = list(generate_lowercase_homoglyphs_variants_for_string(normalized))
        assert letter not in variants, f'{letter} had character {character} which is already considered confusable'
    ...
    assert in_common == set(), f'{letter} had confusables already seen elsewhere: {in_common}'
