...
def compatibility_report(index=None):
    ...
    qs = UpdateCount.objects.filter(
        addon__appsupport__app=amo.FIREFOX.id,
    ...
    for chunk in chunked(updates.items(), 50):
        ...
        for addon in Addon.objects.filter(id__in=chunk):
            if (
                amo.FIREFOX not in addon.compatible_apps
            ...
            reports = (
                CompatReport.objects.filter(
                    guid=addon.guid, app_guid=amo.FIREFOX.guid
                )
    ...
    running_total = 0
    for addon, count in sorted(
        updates.items(), key=lambda x: x[1], reverse=True
    ):
    ...
    log.info(
        'Bulk indexing %s compat docs on %s indices'
