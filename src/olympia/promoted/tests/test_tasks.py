...
def test_add_high_adu_extensions_to_notable():
    ...
    PromotedAddonPromotion.objects.create(addon=already_promoted, promoted_group=group)
    promoted_record_exists = addon_factory(
        average_daily_users=lower_adu_threshold + 1, file_kw={'is_signed': True}
    )
    PromotedAddonPromotion.objects.filter(addon=promoted_record_exists).delete()
