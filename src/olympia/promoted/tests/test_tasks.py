...
from ..tasks import add_high_adu_extensions_to_notable
...
def test_add_high_adu_extensions_to_notable_tier_absent_or_no_threshold():
    ...
    UsageTier.objects.create(slug=NOTABLE_API_NAME, lower_adu_threshold=None)
...
def test_add_high_adu_extensions_to_notable():
    ...
    UsageTier.objects.create(
        slug=NOTABLE_API_NAME, lower_adu_threshold=lower_adu_threshold
    )
    ...
    assert not ignored_theme.reload().promoted_groups(currently_approved=False).api_name
    assert 'line' in already_promoted.promoted_groups(currently_approved=False).api_name
    promoted_record_exists.reload()
    assert (
        'notable'
        in promoted_record_exists.promoted_groups(currently_approved=False).api_name
    )
    assert (
        'notable'
        in unlisted_only_extension.promoted_groups(currently_approved=False).api_name
    )
    assert (
        'notable' in mixed_extension.promoted_groups(currently_approved=False).api_name
    )
    assert (
        'notable'
        in deleted_extension.promoted_groups(currently_approved=False).api_name
    )
