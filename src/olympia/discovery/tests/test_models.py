...
class TestDiscoveryItem(TestCase):
    def test_heading_multiple_authors(self):
        ...
        assert item.heading == (
            ...
            u'?{}">'
            u'Sôme Name by Bàr, Firefox user 345</a></span>'
        ).format(item.build_querystring())
    ...
    def test_heading_custom_xss(self):
        ...
        addon = addon_factory(slug=u'somé-slug', name=u'<script>alert(42)</script>')
        user = user_factory(display_name=u'<script>alert(666)</script>')
        addon.addonuser_set.create(user=user)
        item = DiscoveryItem.objects.create(
            addon=addon, custom_heading=u'<script>alert(0)</script>{addon_name}'
        )
        assert item.heading == (
            ...
            u'by &lt;script&gt;alert(666)&lt;/script&gt;</a>'
        ).format(item.build_querystring())
    ...
    def test_heading_non_custom_xss(self):
        addon = addon_factory(slug=u'somé-slug', name=u'<script>alert(43)</script>')
    ...
    def test_heading_text_custom(self):
        ...
        item = DiscoveryItem.objects.create(
            addon=addon,
            custom_heading=(
                'Fancy Héading {start_sub_heading}with '
    ...
    def test_description_non_custom_extension_xss(self):
        ...
        item = DiscoveryItem.objects.create(addon=addon)
        assert item.description == (
            u'<blockquote>' u'Mÿ &lt;script&gt;alert(5)&lt;/script&gt;</blockquote>'
        )
    ...
    def test_primary_hero_shelf(self):
        item = DiscoveryItem.objects.create(addon=addon_factory(), recommendable=True)
