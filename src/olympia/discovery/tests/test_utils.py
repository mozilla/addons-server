...
def test_get_disco_recommendations_overrides(call_recommendation_server):
    ...
    data = {'options': {'promoted': [['@guid1', 100], ['@guid2', 99], ['@guid3', 98],]}}
    call_recommendation_server.assert_called_with(
...
def test_replace_extensions():
    source = [
        ...
        DiscoveryItem(addon=addon_factory(), custom_addon_name=u'replace me tøø'),
        DiscoveryItem(
            addon=addon_factory(type=amo.ADDON_STATICTHEME), custom_addon_name=u'ŋot me'
        ),
        DiscoveryItem(
            addon=addon_factory(type=amo.ADDON_STATICTHEME), custom_addon_name=u'ŋor me'
        ),
        DiscoveryItem(addon=addon_factory(), custom_addon_name=u'probably me'),
        DiscoveryItem(
            addon=addon_factory(type=amo.ADDON_STATICTHEME), custom_addon_name=u'safê'
        ),
    ]
    # Just 2 replacements
    replacements = [
        DiscoveryItem(addon=addon_factory(), custom_addon_name=u'just for you'),
        DiscoveryItem(addon=addon_factory(), custom_addon_name=u'and this øne'),
    ]
    ...
    replacements.append(
        DiscoveryItem(addon=addon_factory(), custom_addon_name=u'extra ône')
    )
    replacements.append(
        DiscoveryItem(addon=addon_factory(), custom_addon_name=u'extra tôo')
    )
    result = replace_extensions(source, replacements)
    assert result == [
        replacements[0],
