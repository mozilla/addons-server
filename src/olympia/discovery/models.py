...
class DiscoveryItem(ModelBase):
    ...
    def heading(self):
        ...
        if self.custom_heading:
            ...
            value = (
                conditional_escape(ugettext(self.custom_heading))
                .replace(u'{start_sub_heading}', u'<span>')
                .replace(u'{end_sub_heading}', u'</span>')
                .replace(u'{addon_name}', addon_link)
            )
        else:
            value = format_html(
    ...
    def description(self):
        ...
        if self.custom_description:
            ...
            if addon.type == amo.ADDON_EXTENSION and addon.summary:
                ...
                value = u''
        return (
            format_html(u'<blockquote>{}</blockquote>', value)
            if value
            else value
        )
