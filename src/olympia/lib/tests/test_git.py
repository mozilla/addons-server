...
@pytest.mark.parametrize(
    'filename', ['webextension_no_id.xpi', 'webextension_no_id.zip', 'search.xml',]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (MagicMock(type='blob'), 'empty_bat.exe', 'binary', 'application/x-dosexec'),
        ...
        (MagicMock(type='tree'), 'foo', 'directory', 'application/octet-stream'),
        (MagicMock(type='blob'), 'image-svg-without-xml.svg', 'image', 'image/svg+xml'),
        ...
        (MagicMock(type='blob'), 'html-containing.json', 'text', 'application/json'),
    ],
)
def test_get_mime_type_for_blob(entry, filename, expected_category, expected_mimetype):
    root = os.path.join(
        settings.ROOT, 'src/olympia/files/fixtures/files/file_viewer_filetypes/'
    )
