...
class UserDeleteForm(forms.Form):
    email = forms.CharField(
        max_length=255, required=True, widget=RequiredEmailInput
    )
    ...
    def clean(self):
        ...
        if amouser.is_developer:
            ...
            # user is a developer
            log.warning(
                u'[Tampering] Attempt to delete developer account (%s)'
                % self.request.user
            )
            raise forms.ValidationError("")
...
class UserEditForm(forms.ModelForm):
    ...
    display_name = forms.CharField(
        label=_(u'Display Name'), max_length=50, required=False
    )
    location = forms.CharField(
        label=_(u'Location'), max_length=100, required=False
    )
    occupation = forms.CharField(
        label=_(u'Occupation'), max_length=100, required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance:
            ...
            default = {
                ...
                for idx, notification in notifications.NOTIFICATIONS_BY_ID.items()
            }
            user = {
            ...
            saved = self.instance.notifications.values_list(
    ...
    def clean_photo(self):
        ...
        if (
            photo.content_type not in amo.IMG_TYPES
    ...
    def save(self, log_for_developer=True):
        ...
        visible_notifications = (
            notifications.NOTIFICATIONS_BY_ID
            if self.instance.is_developer
...
class AdminUserEditForm(UserEditForm):
    ...
    def changes(self):
        ...
        details = dict(
            [
                (k, (self.initial[k], self.cleaned_data[k]))
    ...
    def clean_anonymize(self):
        if self.cleaned_data['anonymize'] and self.changed_fields() != set(
            ['anonymize']
        ):
            raise forms.ValidationError(
                ugettext(
    ...
    def save(self, *args, **kw):
        ...
        if self.cleaned_data['anonymize']:
            ...
            ActivityLog.create(
                amo.LOG.ADMIN_USER_EDITED,
                self.instance,
                self.cleaned_data['admin_log'],
                details=self.changes(),
            )
            log.info(
                'Admin edit user: %s changed fields: %s'
...
class DeniedNameAddForm(forms.Form):
    ...
    def clean_names(self):
        names = os.linesep.join(
            [
                s.strip()
                for s in self.cleaned_data['names'].splitlines()
                if s.strip()
            ]
        )
        return names
