...
def user_view(f):
    ...
    def wrapper(request, user_id, *args, **kw):
        ...
        if user_id.isdigit():
            ...
            if user_id == 'me' and request.user and request.user.username:
...
def delete(request):
    ...
    return render(
        request, 'users/delete.html', {'form': form, 'amouser': amouser}
    )
...
def delete_photo(request, user_id):
    ...
    if not_mine and not acl.action_allowed(
...
def edit(request):
    ...
    if request.method == 'POST':
        ...
        form = forms.UserEditForm(
            request.POST, request.FILES, request=request, instance=amouser
        )
        if form.is_valid():
            ...
            messages.error(
                ...
                ugettext(
                    'There were errors in the changes you made. '
    ...
    return render(
        request, 'users/edit.html', {'form': form, 'amouser': amouser}
    )
