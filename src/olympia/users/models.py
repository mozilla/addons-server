...
class UserManager(BaseUserManager, ManagerBase):
    def create_user(self, username, email, fxa_id=None, **kwargs):
        ...
        now = timezone.now()
        user = self.model(
            username=username, email=email, fxa_id=fxa_id, last_login=now, **kwargs
        )
        if username is None:
            user.anonymize_username()
        log.debug('Creating user with email {} and username {}'.format(email, username))
...
class UserProfile(OnChangeMixin, ModelBase, AbstractBaseUser):
    ...
    def delete(self, hard=False, ban_user=False):
        ...
        self.delete_picture(
            picture_path=picture_path, original_picture_path=original_picture_path
        )
...
class IPNetworkUserRestriction(ModelBase):
    ...
    def allow_request(cls, request):
        ...
        try:
            ...
            if request.user:
                user_last_login_ip = ipaddress.ip_address(request.user.last_login_ip)
        except ValueError:
...
class EmailReputationRestriction(NormalizeEmailMixin, ReputationRestrictionMixin):
    error_message = EmailUserRestriction.error_message
