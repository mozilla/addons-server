...
class TestUserProfile(TestCase):
    ...
    def test_ban_and_disable_related_content_bulk(
        ...
        if hard_block_addons:
            assert banned_content.blocked_addons_submissions.get() == submission
        else:
            assert not banned_content.blocked_addons_submissions.exists()
    ...
    def test_ban_and_disable_related_content_bulk_with_hard_block_existing_harden(self):
        user_factory(pk=settings.TASK_USER_ID)
        fake_admin = user_factory(display_name='Fake Admin')
        core.set_user(fake_admin)
        user = user_factory(display_name='Ban Me Please')
        soft_blocked_addon = addon_factory(
            name='Was soft blocked', users=[user], average_daily_users=1
        )
        block_factory(
        ...
        regular_addon = addon_factory(
            name='Regular', users=[user], average_daily_users=2
        )
        ...
        user.reload()
        assert user.banned
        for addon in [regular_addon, soft_blocked_addon]:
            version = addon.current_version
            addon.reload()
            version.reload()
            assert addon.status == amo.STATUS_DISABLED
            # We should have 1 BlocklistSubmissions for each add-on: one for
            # blocking, one for hardening.
            submission = BlocklistSubmission.objects.get(input_guids=addon.guid)
            assert submission.changed_version_ids == [version.pk]
            assert (
                submission.signoff_state == BlocklistSubmission.SIGNOFF_STATES.PUBLISHED
            )
        ...
        assert (
            BlocklistSubmission.objects.get(input_guids=soft_blocked_addon.guid).action
            == BlocklistSubmission.ACTIONS.HARDEN
        )

        assert set(
            user.content_disabled_on_ban.blocked_addons_submissions.all()
        ) == set(BlocklistSubmission.objects.all())

    def test_ban_and_disable_related_content_bulk_with_hard_block_authors_mixup(self):
        user_factory(pk=settings.TASK_USER_ID)
        fake_admin = user_factory(display_name='Fake Admin')
        core.set_user(fake_admin)

        user1 = user_factory(display_name='Ban Me Please')
        user2 = user_factory(display_name='Also Ban Me Please')
        addon1 = addon_factory(
            name='First Addon', users=[user1, user2], average_daily_users=1
        )
        addon2 = addon_factory(
            name='Second Addon', users=[user1, user2], average_daily_users=2
        )
        UserProfile.objects.filter(
            pk__in=[user1.pk, user2.pk]
        ).ban_and_disable_related_content(hard_block_addons=True)
        ...
        # want to track them individually in case one user gets unbanned
        # and not the other.
        assert BlocklistSubmission.objects.count() == 2
        for submission in BlocklistSubmission.objects.all():
            assert sorted(submission.input_guids.split('\r\n')) == sorted(
                [addon1.guid, addon2.guid]
            )
            assert submission.changed_version_ids == [
                addon2.versions.get().pk,
                addon1.versions.get().pk,
            ]
            assert (
                submission.signoff_state == BlocklistSubmission.SIGNOFF_STATES.PUBLISHED
            )
        ...
        assert (
            user1.content_disabled_on_ban.blocked_addons_submissions.all()
    ...
    def test_ban_and_disable_related_content_bulk_with_hard_block_conflict_hold(self):
        ...
        user = user_factory(display_name='Ban Me Please')
        addon = addon_factory(
            name='Already has a submission', users=[user], average_daily_users=1
        )
        existing_submission = BlocklistSubmission.objects.create(
