...
class TestMigrateWebextensionsToGitStorage(TestCase):
    ...
    def test_migrate_versions_extracts_source(self, extract_source_mock, extract_mock):
        addon = addon_factory(file_kw={'filename': 'webextension_no_id.xpi'})
        version_to_migrate = addon.current_version
        ...
        extract_mock.assert_has_calls(
            [mock.call(version_to_migrate.pk), mock.call(version_without_source.pk)]
        )
...
def test_repack_themes_for_69():
    ...
    deprecated_theme = addon_factory(name='deprecated', version_kw={'version': '1.0'})
    ...
    with mock.patch(to_mock) as new_theme_version_with_69_properties_mock:
        with mock.patch('olympia.addons.tasks.parse_addon') as parse_mock:
            ...
            repack_themes_for_69(
                [broken_theme.id, already_theme.id, deprecated_theme.id,]
            )
