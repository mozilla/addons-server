...
class TestAddonManager(TestCase):
    ...
    def test_listed(self):
        ...
        assert q.count() == 3
        assert (
            Addon.objects.listed(
                amo.FIREFOX, amo.STATUS_PUBLIC, amo.STATUS_NOMINATED
            ).count()
            == 4
        )
    ...
    def test_new_featured(self):
        ...
        assert f.count() == 3
        assert sorted(x.id for x in f) == ([2464, 7661, 15679])
...
class TestAddonModels(TestCase):
    ...
    def test_transformer(self):
        ...
        new_author = AddonUser.objects.create(
            addon_id=3615,
            user=UserProfile.objects.create(username='abda'),
            listed=True,
        ).user
    ...
    def test_delete_deletes_preview_files(
        self, dpf_vesions_mock, dpf_addons_mock
    ):
        addon = addon_factory()
    ...
    def test_get_featured_by_app(self):
        ...
        FeaturedCollection.objects.create(
            collection=pt_coll, application=amo.FIREFOX.id, locale='pt-PT'
        )
        ...
        FeaturedCollection.objects.create(
            collection=mobile_coll, application=amo.ANDROID.id, locale='pt-PT'
        )
    ...
    def test_app_categories(self):
        ...
        thunderbird_static_cat = CATEGORIES[amo.THUNDERBIRD.id][
            amo.ADDON_EXTENSION
        ]['tags']
...
class TestAddonDelete(TestCase):
    def test_cascades(self):
        ...
        AddonDependency.objects.create(addon=addon, dependent_addon=addon)
...
class TestUpdateStatus(TestCase):
    ...
    def test_unlisted_versions_ignored(self):
        ...
        addon.update_status()
        assert Addon.objects.get(pk=addon.pk).status == (amo.STATUS_PUBLIC)
...
class TestAddonFromUpload(UploadTest):
    ...
    def setUp(self):
        ...
        self.dummy_parsed_data = {'guid': 'guid@xpi', 'version': '0.1'}
...
class TestTrackAddonStatusChange(TestCase):
    ...
    def test_ignore_non_status_changes(self):
        ...
        assert not mock_.called, 'Unexpected call: {}'.format(
            self.mock_incr.call_args
        )
...
class TestLanguagePack(TestCase, amo.tests.AMOPaths):
    def setUp(self):
        ...
        self.addon = amo.tests.addon_factory(
            type=amo.ADDON_LPAPP, status=amo.STATUS_PUBLIC
        )
...
class TestCompatOverride(TestCase):
    def setUp(self):
        ...
        CompatOverrideRange.objects.create(
            compat=two, app=self.app.id, min_version='1', max_version='2'
        )
        CompatOverrideRange.objects.create(
            compat=two,
            app=self.app.id,
    ...
    def test_transformer(self):
        compats = list(
            CompatOverride.objects.transform(CompatOverride.transformer)
        )
    ...
    def test_collapsed_ranges_different_types(self):
        ...
        c = CompatOverride.objects.get(guid='one')
        CompatOverrideRange.objects.create(
            compat=c, app=1, type=0, min_app_version='3', max_app_version='3.*'
        )
        r = c.collapsed_ranges()
    ...
    def test_collapsed_ranges_multiple_versions_and_apps(self):
        c = CompatOverride.objects.get(guid='two')
        CompatOverrideRange.objects.create(
            min_version='5', max_version='6', compat=c, app=1
        )
        r = c.collapsed_ranges()
...
class TestIncompatibleVersions(TestCase):
    ...
    def test_signals_min(self):
        ...
        CompatOverrideRange.objects.create(
            compat=c, app=self.app.id, min_version='0', max_version='1.0'
        )

        # Test the max version matched.
        version1 = Version.objects.create(
            id=2, addon=self.addon, version='1.0'
        )
        assert (
            IncompatibleVersions.objects.filter(version=version1).count() == 1
        )
        assert IncompatibleVersions.objects.count() == 1

        # Test the lower range.
        version2 = Version.objects.create(
            id=1, addon=self.addon, version='0.5'
        )
        assert (
            IncompatibleVersions.objects.filter(version=version2).count() == 1
        )
    ...
    def test_signals_max(self):
        ...
        c = CompatOverride.objects.create(guid='r@b')
        CompatOverrideRange.objects.create(
            compat=c, app=self.app.id, min_version='1.0', max_version='*'
        )
...
class TestMigratedLWTModel(TestCase):
    def setUp(self):
        ...
        MigratedLWT.objects.create(
            lightweight_theme=self.lwt, static_theme=self.static_theme
        )
