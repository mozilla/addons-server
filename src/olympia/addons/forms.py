...
def clean_addon_slug(slug, instance):
    ...
    if slug != instance.slug:
        ...
        if DeniedSlug.blocked(slug):
            msg = ugettext(
                u'The slug cannot be "%(slug)s". ' u'Please choose another.'
            )
...
def clean_tags(request, tags):
    ...
    if denied:
        ...
        msg = ungettext(
            'Invalid tag: {0}', 'Invalid tags: {0}', len(denied)
        ).format(', '.join(denied))
...
class AddonFormBase(TranslationFormMixin, forms.ModelForm):
    ...
    def clean_name(self):
        ...
        name = verify_mozilla_trademark(
            self.cleaned_data['name'], user, form=self
        )
    ...
    def get_tags(self, addon):
        if acl.action_allowed(self.request, amo.permissions.ADDONS_EDIT):
            ...
            return list(
                addon.tags.filter(restricted=False).values_list(
                    'tag_text', flat=True
                )
            )
...
class CategoryForm(forms.Form):
    application = forms.TypedChoiceField(
        amo.APPS_CHOICES, coerce=int, widget=forms.HiddenInput, required=True
    )
    ...
    def save(self, addon):
        ...
        categories_old = [
            c.id
            for c in addon.app_categories.get(amo.APP_IDS[application], [])
        ]
...
class AddonFormMedia(AddonFormBase):
    icon_type = forms.CharField(
        widget=IconTypeSelect(choices=[]), required=False
    )
        ...
        for field in ('header',):
            self.fields[field].widget.attrs = {
                'data-upload-url': reverse(
                    'devhub.personas.upload_persona',
...
class ThemeForm(ThemeFormBase):
    ...
    category = forms.ModelChoiceField(
        queryset=Category.objects.all(), widget=forms.widgets.RadioSelect
    )
    ...
    accentcolor = ColorField(
        required=False, widget=forms.TextInput(attrs={'class': 'color-picker'})
    )
    ...
    def save(self, commit=False):
        ...
        addon._current_version = Version.objects.create(
            addon=addon, version='0'
        )
        addon.save()
...
class EditThemeForm(AddonFormBase):
    ...
    def __init__(self, *args, **kw):
        ...
        cats = sorted(
            Category.objects.filter(type=amo.ADDON_PERSONA, weight__gte=0),
            key=lambda x: x.name,
        )
        self.fields['category'].choices = [(c.id, c.name) for c in cats]
        try:
        ...
        for field in ('header',):
            self.fields[field].widget.attrs = {
                'data-upload-url': reverse(
                    'devhub.personas.reupload_persona',
    ...
    def save(self):
        ...
        changed = (
            set(self.old_tags) != data['tags']
            or self.initial['slug'] != data['slug']  # Check if tags changed.
            or transfield_changed(  # Check if slug changed.
                'description', self.initial, data
            )
    ...
    def save(self):
        ...
        if data.get('owner'):
            ...
            if not self.instance_addonuser:
                ...
                self.instance.addonuser_set.create(
                    user=data['owner'], role=amo.AUTHOR_ROLE_OWNER
                )
