...
class Command(BaseCommand):
    help = ('Backfill the ReusedGuid model with guids of add-ons that have '
    ...
    def handle(self, *args, **options):
        ...
        for pk, fake_guid in addons:
            ...
            if reused_by_pk in addons_pks:
                ...
                log.info('Addon id [%s] also reused, so deferring %s.',
                         reused_by_pk, pks_to_add)
                other_previous_pks[reused_by_pk] = pks_to_add
            else:
                try:
                    real_guid = Addon.unfiltered.get(pk=reused_by_pk).guid
                except Addon.DoesNotExist:
                    log.info('Addon id [%s] not found so no guid to backfill '
                             '%s.', reused_by_pk, pks_to_add)
                    continue
                log.info('Addons %s being added with guid [%s].', pks_to_add,
                         real_guid)
                ReusedGUID.objects.bulk_create(
                    (ReusedGUID(addon_id=pk, guid=real_guid)
