...
urlpatterns = [
    ...
    url(
        ...
        lambda r, addon_id: redirect(
            'discovery.addons.detail', addon_id, permanent=True
        ),
    ),
    url('^addon/%s/' % ADDON_ID, include(addon_patterns)),
    url('^pane/account$', views.pane_account, name='discovery.pane.account'),
    url(
        '^pane/(?P<section>featured|up-and-coming)/%s$'
        % (browser_re + compat_mode_re),
        views.pane_more_addons,
        name='discovery.pane.more_addons',
    ),
    url('^%s$' % (browser_re + compat_mode_re), pane_redirect),
    url(
        '^pane/%s$' % (browser_re + compat_mode_re),
        views.pane,
        name='discovery.pane',
    ),
    url(
        '^pane/promos/%s$' % (browser_re + compat_mode_re),
        views.pane_promos,
