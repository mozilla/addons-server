...
def test_get_addons_from_adzerk_full():
    responses.add(responses.POST, settings.ADZERK_URL, json=adzerk_json)
...
def test_call_adzerk_server(statsd_mock):
    ...
    assert responses.calls[0].request.body == force_bytes(json.dumps(data))
...
def test_get_addons_from_adzerk(call_server_mock, process_mock):
    ...
    call_server_mock.assert_called_with(settings.ADZERK_URL, data)
    ...
    call_server_mock.assert_called_with(settings.ADZERK_URL, data)
    process_mock.assert_called_with({'something': 'something'})
...
def test_send_impression_pings(incr_mock):
    ...
    responses.add(responses.GET, settings.ADZERK_EVENT_URL + 'i.gif?e=eyJ2IjLtg&Bug')
    responses.add(responses.GET, settings.ADZERK_EVENT_URL + 'i.gif?e=eyJ2IjoNtSz8')
...
def test_send_event_ping(incr_mock):
    ...
    responses.add(responses.GET, settings.ADZERK_EVENT_URL + 'r?e=eyJ2IjLtg&Bug')
...
def test_get_signed_impression_blob_from_results():
    signer = TimestampSigner()
    results = {
        '66': {'addon_id': 66, 'impression': '123456', 'click': 'abcdef'},
        '99': {'addon_id': 99, 'impression': '012345', 'click': 'bcdefg'},
    }
