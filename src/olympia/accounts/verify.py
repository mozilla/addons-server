...
def get_fxa_token(code, config):
    ...
    if response.status_code == 200:
        ...
        if data.get('access_token'):
            ...
            log.info(
                'No token returned {data} [{code}]'.format(
                    data=data, code=code
                )
            )
...
def get_fxa_profile(token, config):
    ...
    if response.status_code == 200:
        ...
        if profile.get('email'):
            log.debug(
                'Got profile {profile} [{token}]'.format(
            ...
            log.info(
                'Incomplete profile {profile} [{token}]'.format(
                    profile=profile, token=token
                )
            )
            raise IdentificationError(
        ...
        log.info(
            ...
            '[{token}]'.format(
                token=token, status=response.status_code, body=response.content
            )
        )
        raise IdentificationError(
