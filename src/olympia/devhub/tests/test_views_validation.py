...
class TestUploadValidation(ValidatorTestCase, BaseUploadTest):
    ...
    def test_upload_processed_validation_error(self):
        ...
        response = self.client.post(
            ...
            {'name': 'addon.xpi', 'upload': addon_file},
        )
        ...
        assert upload.processed_validation['messages'][0]['id'] == [
            u'validation',
            u'messages',
            u'legacy_addons_restricted',
        ]
        assert upload.processed_validation['messages'][1]['id'] == [
            u'testcases_content',
            u'test_packed_packages',
...
class TestFileValidation(TestCase):
    ...
    def test_results_page(self):
        ...
        assert not doc('#site-nav').hasClass(
            'app-nav'
...
class TestUploadURLs(TestCase):
    ...
    def setUp(self):
        ...
        self.addon = Addon.objects.create(
            guid='thing@stuff', slug='thing-stuff', status=amo.STATUS_PUBLIC
        )
    ...
    def upload(self, view, **kw):
        ...
        with open(fpath) as file_:
            resp = self.client.post(
                reverse(view, kwargs=kw), {'upload': file_}
            )
...
class TestValidateFile(BaseUploadTest):
    ...
    def setUp(self):
        ...
        with storage.open(self.file.file_path, 'w') as dest:
            shutil.copyfileobj(
                open(self.file_path('invalid-id-20101206.xpi')), dest
            )
        self.addon = self.file.version.addon
    ...
    def test_time(self):
        response = self.client.post(
            reverse(
                'devhub.file_validation', args=[self.addon.slug, self.file.id]
            ),
            follow=True,
        )
        doc = pq(response.content)
...
class TestCompatibilityResults(TestCase):
    ...
    def validate(self, expected_status=200):
        response = self.client.post(
            reverse(
                'devhub.json_bulk_compat_result',
...
class TestUploadCompatCheck(BaseUploadTest):
    ...
    def setUp(self):
        ...
        self.appver = AppVersion.objects.get(
            application=self.app.id, version='3.7a1pre'
        )
