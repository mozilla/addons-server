...
class TestEditAuthor(TestOwnership):
    ...
    def test_deleted_role_not_available(self):
        ...
        assert {
            'role': ['Select a valid choice. 6 is not one of the available choices.']
        } in user_form.errors
        second_author.reload()
...
class TestAuthorInvitation(TestCase):
    ...
    def test_post_accept(self):
        assert not AddonUser.objects.filter(addon=self.addon, user=self.user).exists()
    ...
    def test_post_accept_deleted_before(self):
        ...
        assert not AddonUser.objects.filter(addon=self.addon, user=self.user).exists()
        assert AddonUser.unfiltered.filter(addon=self.addon, user=self.user).exists()
