    ...
    def test_counts(self):
        ...
        for _ in range(10):
            Rating.objects.create(
                addon=addon, user=user, version=addon.current_version
            )
...
class TestDelete(TestCase):
    ...
    def test_post(self):
        ...
        response = self.client.post(
            self.get_url(), {'slug': 'addon-slug'}, follow=True
        )
        assert pq(response.content)('.notification-box').text() == (
            'Add-on deleted.'
        )
    ...
    def test_post_wrong_slug(self):
        self.get_addon().get().update(slug='addon-slug')
        response = self.client.post(
            self.get_url(), {'slug': 'theme-slug'}, follow=True
        )
        assert pq(response.content)('.notification-box').text() == (
            'URL name was incorrect. Add-on was not deleted.'
        )
...
class TestHome(TestCase):
    ...
    def test_my_addons_persona_versions_link(self):
        ...
        span_text = (
            ...
            .eq(0)
            .find('span.DevHub-MyAddons-VersionStatus')
            .text()
        )
    ...
    def test_my_addons(self):
        statuses = [
            ...
            (amo.STATUS_PUBLIC, amo.STATUS_AWAITING_REVIEW, 'Approved'),
            (amo.STATUS_DISABLED, amo.STATUS_PUBLIC, 'Disabled by Mozilla'),
        ]
        ...
        assert (
            self.get_pq()('.DevHub-MyAddons-list .DevHub-MyAddons-item').length
            == 0
        )
...
class TestAPIAgreement(TestCase):
    ...
    def test_agreement_read_but_too_long_ago(self):
        ...
        before_agreement_last_changed = datetime(
            2018, 1, 1, 12, 0
        ) - timedelta(days=1)
...
class TestUploadDetail(BaseUploadTest):
    ...
    def test_android_excludes_desktop_platforms(self):
        self.check_excluded_platforms(
            'android-phone.xpi', [str(p) for p in amo.DESKTOP_PLATFORMS]
        )

    def test_search_tool_excludes_all_platforms(self):
        self.check_excluded_platforms(
            'searchgeek-20090701.xml',
            [str(p) for p in amo.SUPPORTED_PLATFORMS],
        )

    def test_desktop_excludes_mobile(self):
        self.check_excluded_platforms(
            'desktop.xpi', [str(p) for p in amo.MOBILE_PLATFORMS]
        )
    ...
    def test_webextension_android_excluded_if_no_48_support(self):
        ...
        self.check_excluded_platforms(
            'valid_webextension_max_47.xpi', [str(amo.PLATFORM_ANDROID.id)]
        )
    ...
    def test_static_theme_supports_all_desktop_platforms(self):
        ...
        self.check_excluded_platforms(
            'static_theme.zip', [str(amo.PLATFORM_ANDROID.id)]
        )
...
class TestRequestReview(TestCase):
    ...
    def setUp(self):
        ...
        self.public_url = reverse(
            'devhub.request-review', args=[self.addon.slug]
        )
        assert self.client.login(email='admin@mozilla.com')
...
class TestHasCompleteMetadataRedirects(TestCase):
    ...
    def setUp(self):
        ...
        self.addon = Addon.objects.get(id=3615)
        assert (
            self.addon.has_complete_metadata()
