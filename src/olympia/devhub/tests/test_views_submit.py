...
from olympia.addons.models import Addon, AddonCategory, AddonReviewerFlags, Category
...
class TestAddonSubmitUpload(UploadTest, TestCase):
    ...
    def test_one_xpi_for_multiple_apps_unlisted_addon(self, mock_auto_sign_file):
        assert Addon.objects.count() == 0
        response = self.post(compatible_apps=[amo.FIREFOX, amo.ANDROID], listed=False)
...
class TestAddonSubmitSource(TestSubmitBase):
    ...
    def test_submit_source_commits_to_git_asnychronously(self, extract_mock):
        ...
        extract_mock.assert_called_once_with(
            version_id=self.addon.current_version.pk, author_id=self.user.pk
        )
...
class TestVersionSubmitUploadListed(VersionSubmitUploadMixin, UploadTest):
    ...
    def test_experiments_inside_webext_are_auto_signed(self, mock_sign_file):
        ...
        self.grant_permission(self.user, ':'.join(amo.permissions.EXPERIMENTS_SUBMIT))
    ...
    @mock.patch('olympia.devhub.views.sign_file')
    def test_experiment_inside_webext_upload_without_permission(self, mock_sign_file):
        self.upload = self.get_upload(
            'experiment_inside_webextension.xpi',
            validation=json.dumps(
                {
...
class TestVersionSubmitDetails(TestSubmitBase):
    ...
    def test_clear_request_for_information(self):
        ...
        response = self.client.post(self.url, {'clear_pending_info_request': True})
        self.assert3xx(
            response,
            reverse(
                'devhub.submit.version.finish', args=[self.addon.slug, self.version.pk]
