...
class TestUploadToRemoteSettings(TestCase):
    ...
    def test_skip_update_unless_force_base(self):
        ...
        assert mlbf.storage.exists(mlbf.filter_path(MLBFDataType.BLOCKED))
        assert not mlbf.storage.exists(mlbf.stash_path(MLBFDataType.BLOCKED))
    ...
    def test_upload_stash_unless_force_base(self):
        ...
        mlbf = MLBF.load_from_storage(self.current_time)
        assert mlbf.storage.exists(mlbf.filter_path(MLBFDataType.BLOCKED)) == force_base
        assert mlbf.storage.exists(mlbf.stash_path(MLBFDataType.BLOCKED)) != force_base
    ...
    def test_upload_stash_unless_missing_base_filter(self):
        ...
        mlbf = MLBF.load_from_storage(self.current_time)
        assert not mlbf.storage.exists(mlbf.filter_path(MLBFDataType.BLOCKED))
        assert mlbf.storage.exists(mlbf.stash_path(MLBFDataType.BLOCKED))
    ...
    def test_upload_stash_unless_enough_changes(self):
        ...
        assert new_mlbf.storage.exists(new_mlbf.filter_path(MLBFDataType.BLOCKED))
        assert not new_mlbf.storage.exists(new_mlbf.stash_path(MLBFDataType.BLOCKED))
