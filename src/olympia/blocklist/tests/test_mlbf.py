...
class TestMLBF(TestCase):
    ...
    def setUp(self):
        ...
        self.over = Block.objects.create(
            addon=addon_factory(file_kw={'hash': 'overhash'}),
            updated_by=user_factory(),
            max_version='0')
        self.under = Block.objects.create(
            addon=addon_factory(file_kw={'hash': 'underhash'}),
            updated_by=user_factory(),
            min_version='9999')
        self.over_tuple = (
            self.over.guid,
            self.over.addon.current_version.version,
            self.over.addon.current_version.all_files[0].hash)
        self.under_tuple = (
            self.under.guid,
            self.under.addon.current_version.version,
            self.under.addon.current_version.all_files[0].hash)
    ...
    def test_all_guids(self):
        ...
        assert self.over_tuple in all_guids
        assert self.under_tuple in all_guids
    ...
    def test_get_blocked_guids(self):
        ...
        assert self.over_tuple not in blocked_guids
        assert self.under_tuple not in blocked_guids

    def test_hash_filter_inputs(self):
        data = [
            ('guid@', '1.0', 'uduhdu'),
            ('foo@baa', '999.223a', 'duowoij'),
        ]
        assert hash_filter_inputs(data, get_mlbf_key_format()) == [
            'guid@:1.0:uduhdu',
            'foo@baa:999.223a:duowoij',
        ]
    ...
    def test_generate_mlbf(self):
        ...
        blocked = [
            ('guid1@', '1.0', '1'), ('@guid2', '1.0', '2'),
            ('@guid2', '1.1', '3'), ('guid3@', '0.01b1', '4')]
        not_blocked = [
            ('guid10@', '1.0', '5'), ('@guid20', '1.0', '6'),
            ('@guid20', '1.1', '7'), ('guid30@', '0.01b1', '8'),
            ('guid100@', '1.0', '9'), ('@guid200', '1.0', 'A'),
            ('@guid200', '1.1', 'B'), ('guid300@', '0.01b1', 'C')]
        bfilter = generate_mlbf(
            stats, key_format, blocked=blocked, not_blocked=not_blocked)
        for entry in blocked:
            key = key_format.format(
                guid=entry[0], version=entry[1], xpi_hash=entry[2])
