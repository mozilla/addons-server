...
from .forms import BLSubmissionForm, MultiAddForm, MultiDeleteForm
from .models import Block, BLSubmission
from .tasks import process_blsubmission
...
@admin.register(BLSubmission)
class BLSubmissionAdmin(admin.ModelAdmin):
    list_display = (
        'blocks_count',
        'action',
        'signoff_state',
    ...
    change_form_template = 'blocklist/blsubmission_change_form.html'
    form = BLSubmissionForm

    class Media:
        css = {
            'all': ('css/admin/blocklist_blsubmission.css',)
        }
        js = ('js/i18n/en-US.js',)

    def has_delete_permission(self, request, obj=None):
        # For now, keep all BLSubmission records.
        # TODO: define under what cirumstances records can be safely deleted.
        return False

    def is_pending_signoff(self, obj):
        return obj and obj.signoff_state == BLSubmission.SIGNOFF_PENDING
    ...
    def is_add_change_submission(self, request, obj):
        return (
            ...
            str(BLSubmission.ACTION_ADDCHANGE))
    ...
    def has_signoff_approve_permission(self, request, obj=None):
        ...
        available on the change form.  `BLSubmission.can_user_signoff`
    ...
    def get_fieldsets(self, request, obj):
        ...
        elif obj.signoff_state == BLSubmission.SIGNOFF_PUBLISHED:
    ...
    def add_view(self, request, **kwargs):
        ...
        return TemplateResponse(
            request, 'blocklist/blsubmission_add_form.html', context)
    ...
    def save_model(self, request, obj, form, change):
        if change and self.is_pending_signoff(obj):
            ...
            if is_approve:
                if not self.has_signoff_approve_permission(request, obj):
                    raise PermissionDenied
                obj.signoff_state = BLSubmission.SIGNOFF_APPROVED
                obj.signoff_by = request.user
            elif is_reject:
                if not self.has_signoff_reject_permission(request, obj):
                    raise PermissionDenied
                obj.signoff_state = BLSubmission.SIGNOFF_REJECTED
        ...
        if obj.is_submission_ready:
            ...
            process_blsubmission.delay(obj.id)
...
class BlockAdmin(BlockAdminAddMixin, admin.ModelAdmin):
    ...
    def block_history(self, obj):
        submission = BLSubmission.get_submissions_from_guid(
            obj.guid).first()
        ...
        return render_to_string(
            ...
            {'logs': logs, 'blsubmission': submission})
    ...
    def has_change_permission(self, request, obj=None):
        if obj and BLSubmission.get_submissions_from_guid(obj.guid):
            return False
        else:
