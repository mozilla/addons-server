...
urlpatterns = [
    ...
    # Site events data.
    url(
        '^statistics/events-(?P<start>\d{8})-(?P<end>\d{8})\.json$',
        stats_views.site_events,
        name='amo.site_events',
    ),
    ...
    url('^pages/about$', lambda r: redirect('pages.about', permanent=True)),
    # Redirect persona/xxx
    url(
        '^getpersonas$',
        lambda r: redirect(
            'http://www.getpersonas.com/gallery/All/Popular', permanent=True
        ),
    ),
    url(
        '^persona/(?P<persona_id>\d+)',
        addons_views.persona_redirect,
        name='persona',
    ),
    url(
