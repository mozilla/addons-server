...
class ReviewViewSet(AddonChildMixin, ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # The serializer needs reply, version and user. We don't need much for
        # version and user, so we can make joins with select_related(), but for
        # replies additional queries will be made for translations anyway so
        # we're better off using prefetch_related() to make a separate query
        # to fetch them all.
