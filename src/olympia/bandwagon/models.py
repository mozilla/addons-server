...
class Collection(ModelBase):
    ...
    def update_featured_status(sender, instance, **kwargs):
        ...
        addons = kwargs.get('addons', [addon.id for addon in instance.addons.all()])
        if addons:
    ...
    def post_delete(sender, instance, **kwargs):
        ...
        if instance.collection.is_featured():
            Collection.update_featured_status(
                sender, instance.collection, addons=[instance.addon.pk], **kwargs
            )
...
class FeaturedCollection(ModelBase):
    ...
    def __str__(self):
        return u'%s (%s: %s)' % (self.collection, self.application, self.locale)
