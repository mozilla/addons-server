...
class FileSelectWidget(widgets.Select):
    ...
    def render_options(self, context):
        def option(files, label=None, deleted=False, channel=None):
            ...
            status = set(
                u'status-%s' % amo.STATUS_CHOICES_API[f.status] for f in files
            )
            if deleted:
        ...
        for ver in vers.order_by('-created'):
            ...
            if len(distinct_files) == 1:
                ...
                output.extend(
                    (u'<optgroup label="', jinja2.escape(ver.version), u'">')
                )
...
class FileCompareForm(forms.Form):
    left = forms.ModelChoiceField(
        queryset=File.objects.all(), widget=FileSelectWidget
    )
