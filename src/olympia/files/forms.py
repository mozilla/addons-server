...
class FileSelectWidget(widgets.Select):
    def render(self, name, value, attrs=None, renderer=None):
        ...
        output = [
            format_html('<select name="{}"{}>', name, rendered_attrs if attrs else '')
        ]
    ...
    def render_options(self, context):
        def option(files, label=None, deleted=False, channel=None):
            ...
            files = sorted(files, key=lambda a: a.status == amo.STATUS_DISABLED)

            if label is None:
                label = u', '.join(force_text(f.get_platform_display()) for f in files)

            output = [u'<option value="', jinja2.escape(files[0].id), u'" ']
            if selected in files:
                output.append(u' selected="true"')

            status = set(u'status-%s' % amo.STATUS_CHOICES_API[f.status] for f in files)
            if deleted:
        ...
        for ver in vers.order_by('-created'):
            ...
            if len(distinct_files) == 1:
                output.extend(option(distinct_files[0], label, ver.deleted, channel))
            elif distinct_files:
                output.extend((u'<optgroup label="', jinja2.escape(ver.version), u'">'))
                for f in distinct_files:
                    output.extend(option(f, deleted=ver.deleted, channel=channel))
...
class FileCompareForm(forms.Form):
    left = forms.ModelChoiceField(queryset=File.objects.all(), widget=FileSelectWidget)
    right = forms.ModelChoiceField(
        queryset=File.objects.all(), widget=FileSelectWidget, required=False
    )
    ...
    def __init__(self, *args, **kw):
        ...
        if acl.check_unlisted_addons_reviewer(self.request):
            should_show_channel = (
                queryset.filter(version__channel=amo.RELEASE_CHANNEL_LISTED).exists()
                and queryset.filter(
                    version__channel=amo.RELEASE_CHANNEL_UNLISTED
                ).exists()
            )
        else:
            should_show_channel = False
            queryset = queryset.filter(version__channel=amo.RELEASE_CHANNEL_LISTED)
    ...
    def clean(self):
        if (
            not self.errors
            and self.cleaned_data.get('right') == self.cleaned_data['left']
        ):
