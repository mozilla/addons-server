    ...
    def parse(cls, xpi_fobj, minimal=False):
        ...
        if zip_file.exists('manifest.json'):
            data = ManifestJSONExtractor(
                zip_file, certinfo=certificate_info
...
class RDFExtractor(object):
            ...
            data.update(
                {
                    ...
                    'is_restart_required': (
                        ...
                        and self.find('type')
                        not in self.ALWAYS_RESTARTLESS_TYPES
                    ),
                    'apps': self.apps(),
    ...
    def find_type(self):
        ...
        is_complete_theme = self.zip_file.source.name.endswith(
            '.jar'
        ) or self.find('internalName')
    ...
    def apps(self):
        ...
        for ctx in self.rdf.objects(None, self.uri('targetApplication')):
            ...
            if app.guid not in amo.APP_GUIDS or app.id in seen_apps:
                continue
            if app not in amo.APP_USAGE_FIREFOXES_ONLY and switch_is_active(
            ...
            try:
                ...
                needs_max_56_star = (
                    ...
                    and max_appver_text == '*'
                    and not (
                        self.certinfo and self.certinfo.is_mozilla_signed_ou
                    )
                )
...
class ManifestJSONExtractor(object):
    ...
    def apps(self):
        ...
        doesnt_support_no_id = self.strict_min_version and (
        ...
        for app, default_min_version in apps:
            if self.guid is None and not self.strict_min_version:
                strict_min_version = max(
                    amo.DEFAULT_WEBEXT_MIN_VERSION_NO_ID, default_min_version
                )
            else:
                strict_min_version = (
            ...
            # for Firefox).
            skip_app = self.strict_min_version and vint(
                self.strict_min_version
...
def extract_search(content):
    ...
    def text(tag):
        try:
            ...
            raise forms.ValidationError(
                ugettext(
                    'Could not parse uploaded file, missing or empty '
                    '<%s> element'
                )
                % tag
            )
    ...
    def is_valid(self):
        ...
        for info in info_list:
            try:
                ...
                # is obviously broken :-/
                log.error(
                    'Extraction error, invalid file name encoding in '
            ...
            if '..' in info.filename or info.filename.startswith('/'):
                log.error(
                    'Extraction error, invalid file name (%s) in '
            ...
            if info.file_size > settings.FILE_UNZIP_SIZE_LIMIT:
                log.error(
                    'Extraction error, file too big (%s) for file (%s): '
...
def check_xpi_info(xpi_info, addon=None, xpi_file=None, user=None):
    ...
    if not addon and not system_addon_submission_allowed(user, xpi_info):
        ...
        raise forms.ValidationError(
            ugettext(
                u'You cannot submit an add-on with a guid ending '
                u'%s' % guids
            )
        )
...
def _update_version_in_install_rdf(content, new_version_number):
    ...
    for node in tree.xpath(
        '//em:version | //*[@em:version]', namespaces={'em': namespace}
    ):
...
def extract_header_img(file_obj, theme_data, dest_path):
    ...
    header_url = images_dict.get('headerURL', images_dict.get('theme_frame'))
