...
class TestSendMail(TestCase):
    ...
    def test_deny_flag(self):
        ...
        settings.EMAIL_DENY_LIST = (to,)
        success = send_mail(
            'test subject', 'test body', recipient_list=[to], use_deny_list=True
        )
        assert success
        assert len(mail.outbox) == 0

        success = send_mail(
            'test subject', 'test body', recipient_list=[to], use_deny_list=False
        )
        assert success
    ...
    def test_user_mandatory(self):
        ...
        success = send_mail(
            'test subject', 'test body', perm_setting=n, recipient_list=[to]
        )
