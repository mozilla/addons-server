...
@task(
    ...
    retry_backoff=settings.SCANNER_WEBHOOK_RETRY_INITIAL_DELAY,
    # Longest delay between two retries, i.e. the one before the last retry.
    retry_backoff_max=(
        settings.SCANNER_WEBHOOK_RETRY_INITIAL_DELAY * 2 ** (WEBHOOK_MAX_RETRIES - 1)
    ),
    retry_jitter=False,
)
@use_primary_db
def wait_for_scanner_results(self, version_pk):
    """Call the webhooks that still owe us results again, retrying with an
    increasing delay until they answer, then record artificial results matching
    the SCANNER_RESULTS_MISSING rule to stop waiting."""
