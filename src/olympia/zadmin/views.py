...
def compat(request):
    ...
    data = {
        'appver': '%s' % FIREFOX_COMPAT[0]['main'],
        'minimum': minimum,
        'ratio': ratio,
        'type': 'all',
    }
...
def compat_stats(request, version, minimum, ratio, binary):
    ...
    for obj in addons.object_list:
        ...
        obj['has_override'] = (
            obj['overrides']
            .filter(_compat_ranges__min_app_version=version + 'a1')
            .exists()
        )
...
def featured_collection(request):
    ...
    return render(
        request, 'zadmin/featured_collection.html', dict(collection=c)
    )
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if data['recipients'] == 'eula':
            ...
            raise NotImplementedError(
                'If you want to support emailing other '
        ...
        total = 0
        for emails in chunked(
            set(qs.values_list('user__email', flat=True)), 100
        ):
            total += len(emails)
            tasks.admin_email.delay(
                emails,
                data['subject'],
                data['message'],
...
def recalc_hash(request, file_id):
    ...
    messages.success(
        request, 'File hash and size recalculated for file %d.' % file.id
    )
    return {'success': 1}
...
def memcache(request):
    ...
    if cache._cache and hasattr(cache._cache, 'get_stats'):
        ...
        stats = []
    return render(
        request, 'zadmin/memcache.html', {'form': form, 'stats': stats}
    )
...
def site_events(request, event_id=None):
    ...
    return render(
        request, 'zadmin/site_events.html', {'form': form, 'events': events}
    )
