...
class TestPrimaryHero(TestCase):
    ...
    def test_clean_requires_approved_can_primary_hero_group(self):
        ...
        ph.reload()
        ph.enabled = True
        assert RECOMMENDED in ph.promoted_addon.addon.promoted_group()
        ph.clean()  # it raises if there's an error
        ...
        ph.reload()
        ph.enabled = True
        assert STRATEGIC in ph.promoted_addon.addon.promoted_group()
        ...
        ph.reload()
        ph.enabled = True
        assert SPOTLIGHT in ph.promoted_addon.addon.promoted_group()
        ph.clean()  # it raises if there's an error
