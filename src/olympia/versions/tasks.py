...
def generate_static_theme_preview(theme_manifest, version_pk):
    ...
    for size in sizes:
        ...
        if write_svg_to_png(svg, preview.image_path):
            ...
            data = {
                'sizes': {'image': size['full'], 'thumbnail': size['thumbnail'],},
                'colors': colors,
            }
            preview.update(**data)
...
def extract_version_to_git(version_id, author_id=None, note=None):
    ...
    log.info('Extracting {version_id} into git backend'.format(version_id=version_id))
    ...
    log.info(
        'Extracted {version} into {git_path}'.format(
...
def extract_version_source_to_git(version_id, author_id=None):
    ...
    log.info(
        'Extracting {version_id} source into git backend'.format(version_id=version_id)
    )
