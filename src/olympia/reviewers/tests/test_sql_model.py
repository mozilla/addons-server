...
class TestSQLModel(TestCase):
    ...
    def test_slice5(self):
        assert ['defilbrilator'] == [
            c.product for c in ProductDetail.objects.all().order_by('product')[0:1]
        ]
        assert ['life jacket'] == [
            c.product for c in ProductDetail.objects.all().order_by('product')[1:2]
        ]
        assert ['snake skin jacket'] == [
            c.product for c in ProductDetail.objects.all().order_by('product')[2:3]
        ]
    ...
    def test_filter_raw_in(self):
        qs = Summary.objects.all().filter_raw('category IN', ['apparel', 'safety'])
    ...
    def test_crazy_nesting(self):
        qs = ProductDetail.objects.all().filter_raw(
            Q(
                ...
                'product =',
                'defilbrilator',
                Q('product =', 'life jacket') | Q('product =', 'snake skin jacket'),
                'category =',
                'safety',
            )
        )
