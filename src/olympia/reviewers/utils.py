...
class NeedsHumanReviewTable(AutoApprovedTable):
    def render_addon_name(self, record):
        ...
        versions_flagged_by_scanners = record.versions.filter(
            channel=amo.RELEASE_CHANNEL_LISTED, needs_human_review=True
        ).count()
        ...
        unlisted_versions_flagged_by_scanners = record.versions.filter(
            channel=amo.RELEASE_CHANNEL_UNLISTED, needs_human_review=True
        ).count()
...
class ReviewHelper(object):
    ...
    def __init__(
        self, request=None, addon=None, version=None, content_review_only=False
    ):
        self.handler = None
        self.required = {}
        self.addon = addon
    ...
    def get_actions(self, request):
        ...
        regular_content_review_is_allowed = (
            self.content_review_only and not self.addon.needs_admin_content_review
        )
        reviewable_because_not_reserved_for_admins_or_user_is_admin = (
            ...
            or (
                self.version
                and (
                    regular_addon_review_is_allowed or regular_content_review_is_allowed
                )
            )
        )
        reviewable_because_pending = self.version and self.version.is_unreviewed
        ...
        was_auto_approved_and_user_can_post_review = (
            ...
            and not self.content_review_only
        )
        ...
        is_public_and_listed_and_user_can_post_review = (
            self.version
            and self.addon.status == amo.STATUS_APPROVED
            and self.version.channel == amo.RELEASE_CHANNEL_LISTED
            and is_post_reviewer
        )
        is_valid_and_listed_and_user_can_content_review = (
            self.version
            and (self.addon.is_public() or self.addon.is_unreviewed())
            and self.version.channel == amo.RELEASE_CHANNEL_LISTED
            and is_content_reviewer
            and self.content_review_only
        )
        ...
        actions['approve_content'] = {
            ...
            'available': (
                reviewable_because_not_reserved_for_admins_or_user_is_admin
                and is_valid_and_listed_and_user_can_content_review
            ),
        }
        ...
        actions['reject_multiple_versions'] = {
            ...
            'versions': True,
            'details': _(
                'This will reject the selected public '
            ...
            'available': (
                ...
                and (
                    is_public_and_listed_and_user_can_post_review
                    or is_valid_and_listed_and_user_can_content_review
                )
            ),
        }
        actions['reply'] = {
...
class ReviewBase(object):
    def __init__(self, request, addon, version, review_type, content_review_only=False):
    ...
    def reviewer_reply(self):
        ...
        if self.version:
            ...
            if self.data.get('info_request'):
                ...
                info_request_deadline = datetime.now() + timedelta(
                    days=info_request_deadline_days
                )
                ...
                self.addon.addonreviewerflags = AddonReviewerFlags.objects.update_or_create(
                    addon=self.addon,
                    defaults={
    ...
    def reject_multiple_versions(self):
        ...
        for version in self.data['versions']:
            ...
            self.log_action(
                action_id, version=version, files=files, timestamp=timestamp
            )
        ...
        if self.addon.status != amo.STATUS_APPROVED:
            ...
            subject = (
                u'Mozilla Add-ons: %s%s has been disabled on ' u'addons.mozilla.org'
            )
        else:
