...
ACTION_DICT = dict(
    approved=amo.LOG.APPROVE_RATING, deleted=amo.LOG.DELETE_RATING
)
...
class QueueSearchForm(forms.Form):
    ...
    def filter_qs(self, qs):
        ...
        if data['application_id']:
            qs = qs.filter_raw(
                'apps_match.application_id =', data['application_id']
            )
    ...
    def filter_qs(self, qs):
        ...
        if data['application_id']:
            ...
            if data['max_version']:
                ...
                qs.base_query['from'].extend(joins)
                qs = qs.filter_raw(
                    'max_version.version =', data['max_version']
                )
        if data['text_query']:
            lang = get_language()
            joins = [
...
class ReviewForm(forms.Form):
    ...
    operating_systems = forms.CharField(
        required=False, label=_(u'Operating systems:')
    )
    ...
    def __init__(self, *args, **kw):
        ...
        if 'reject_multiple_versions' in self.helper.actions:
            self.fields['versions'].queryset = (
                self.helper.addon.versions.distinct()
                .filter(
...
class ThemeReviewForm(forms.Form):
    theme = forms.ModelChoiceField(
        queryset=Persona.objects.all(), widget=forms.HiddenInput()
    )
    ...
    def clean_reject_reason(self):
        ...
        if (
            self.cleaned_data.get('action') == amo.ACTION_REJECT
    ...
    def clean_comment(self):
        ...
        if not comment and (
            action == amo.ACTION_FLAG
            or action == amo.ACTION_MOREINFO
            or (action == amo.ACTION_REJECT and reject_reason == 0)
        ):
            raise_required()
        return comment
    ...
    def save(self):
        ...
        is_rereview = theme.rereviewqueuetheme_set.exists() and theme.addon.status not in (
            amo.STATUS_PENDING,
...
class ThemeSearchForm(forms.Form):
    q = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={'autocomplete': 'off', 'placeholder': _(u'Search')}
        ),
    )
