...
def ratings_moderation_log_detail(request, id):
    ...
    can_undelete = (
        review
        and review.deleted
        and (is_admin or request.user.pk == log.user.pk)
    )
...
def dashboard(request):
    ...
    if view_all or acl.action_allowed(request, amo.permissions.THEMES_REVIEW):
        sections[ugettext('Lightweight Themes')] = [
            ...
            (
                ugettext('Flagged Themes ({0})').format(
                    Persona.objects.filter(
                        addon__status=amo.STATUS_REVIEW_PENDING
                    ).count()
                ),
    ...
    if view_all or acl.action_allowed(request, amo.permissions.REVIEWS_ADMIN):
        ...
        sections[ugettext('Admin Tools')] = [
            (
                ugettext(
                    'Expired Information Requests ({0})'.format(
                        expired.count()
                    )
                ),
...
def performance(request, user_id=False):
    ...
    def _sum(iter, types, exclude=False):
        ...
        return sum(
            s.total for s in iter if (s.atype in types) == (not exclude)
        )
...
def _performance_by_month(user_id, months=12, end_month=None, end_year=None):
    ...
    start_time = time.mktime(
        (end_year, end_month + 1 - months, 1, 0, 0, 0, 0, 0, -1)
    )

    sql = PerformanceGraph.objects.filter_raw(
        'log_activity.created >=', date.fromtimestamp(start_time).isoformat()
    ).filter_raw(
        'log_activity.created <', date.fromtimestamp(end_time).isoformat()
    )
    ...
    for row in sql.all():
        ...
        if label not in monthly_data:
            ...
            monthly_data[label] = dict(
                teamcount=0, usercount=0, teamamt=0, label=xaxis
            )
...
def filter_static_themes(qs, extension_reviewer, theme_reviewer):
    ...
    return (
        qs.filter_raw('addontype_id IN', types_to_include)
        if types_to_include
        else qs
    )
...
def _queue(
    request, TableObj, tab, qs=None, unlisted=False, SearchForm=QueueSearchForm
):
    if qs is None:
    ...
    if hasattr(qs, 'sql_model'):
        ...
        if not unlisted:
            qs = filter_static_themes(
                qs,
                acl.action_allowed(request, amo.permissions.ADDONS_REVIEW),
                acl.action_allowed(
                    request, amo.permissions.STATIC_THEMES_REVIEW
                ),
            )
            ...
            qs = qs.filter(
                ...
                | Q(
                    **{
                        'addons_addonreviewerflags.auto_approval_disabled': True
                    }
                )
            )
...
def queue_auto_approved(request):
    ...
    return _queue(
        request, AutoApprovedTable, 'auto_approved', qs=qs, SearchForm=None
    )
...
def review(request, addon, channel=None):
    ...
    versions = (
        Version.unfiltered.filter(addon=addon, channel=channel)
    ...
    all_versions = (
        _get_comments_for_hard_deleted_versions(addon)
        if channel == amo.RELEASE_CHANNEL_LISTED
        else []
    )
    ...
    user_changes_log = AddonLog.objects.filter(
        activity_log__action__in=user_changes_actions, addon=addon
    ).order_by('id')
    ctx = context(
        request,
        actions=actions,
...
def queue_version_notes(request, addon_id):
    ...
    return {
        'releasenotes': unicode(version.releasenotes),
