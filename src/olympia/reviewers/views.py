...
def dashboard(request):
    ...
    if view_all or acl.action_allowed(request, amo.permissions.ADDONS_REVIEW):
        ...
        if not admin_reviewer:
            review_queue = filter_admin_review_for_legacy_queue(review_queue)
        ...
        sections[ugettext('Pre-Review Add-ons')].extend(
            (
                (
                    ugettext('Other Pending Review ({0})').format(review_queue.count()),
    ...
    if view_all or acl.action_allowed(request, amo.permissions.ADDONS_POST_REVIEW):
    ...
    if view_all or acl.action_allowed(request, amo.permissions.STATIC_THEMES_REVIEW):
        ...
        if not admin_reviewer:
            full_review_queue = filter_admin_review_for_legacy_queue(full_review_queue)
            pending_queue = filter_admin_review_for_legacy_queue(pending_queue)
    ...
    if view_all or acl.action_allowed(request, amo.permissions.REVIEWS_ADMIN):
        ...
        sections[ugettext('Admin Tools')] = [
            (
                ugettext('Expired Information Requests ({0})'.format(expired.count())),
...
def fetch_queue_counts(admin_reviewer):
    ...
    counts = {
        'extension': construct_query_from_sql_model(ViewExtensionQueue),
        'theme_pending': construct_query_from_sql_model(ViewThemePendingQueue),
        'theme_nominated': construct_query_from_sql_model(ViewThemeFullReviewQueue),
        'recommended': construct_query_from_sql_model(ViewRecommendedQueue),
        'moderated': Rating.objects.all().to_moderate().count,
        'auto_approved': (
            Addon.objects.get_auto_approved_queue(admin_reviewer=admin_reviewer).count
        ),
        'content_review': (
            Addon.objects.get_content_review_queue(admin_reviewer=admin_reviewer).count
        ),
        'needs_human_review': (
            Addon.objects.get_needs_human_review_queue(
                admin_reviewer=admin_reviewer
...
def queue_needs_human_review(request):
    admin_reviewer = is_admin_reviewer(request)
    qs = Addon.objects.get_needs_human_review_queue(admin_reviewer=admin_reviewer)
    return _queue(
        request, NeedsHumanReviewTable, 'needs_human_review', qs=qs, SearchForm=None
    )
...
def perform_review_permission_checks(
    ...
    """
    unlisted_only = (
        channel == amo.RELEASE_CHANNEL_UNLISTED or not addon.has_listed_versions()
    )
    was_auto_approved = (
    ...
    if content_review_only:
        if not acl.action_allowed(request, amo.permissions.ADDONS_CONTENT_REVIEW):
            raise PermissionDenied
    elif static_theme:
...
def review(request, addon, channel=None):
    ...
    deleted_addon_ids = (
        ReusedGUID.objects.filter(guid=addon.guid).values_list('addon_id', flat=True)
        if addon.guid
        else []
    )
...
def download_git_stored_file(request, version_id, filename):
    ...
    serializer = FileEntriesSerializer(
        instance=file, context={'file': filename, 'request': request}
    )
    ...
    response = http.HttpResponse(
        content=actual_blob.data, content_type=selected_file['mimetype']
    )
...
class AddonReviewerViewSet(GenericViewSet):
    ...
    @action(detail=True, methods=['post'], permission_classes=[AllowAnyKindOfReviewer])
    def subscribe(self, request, **kwargs):
        addon = get_object_or_404(Addon, pk=kwargs['pk'])
        ReviewerSubscription.objects.get_or_create(user=request.user, addon=addon)
        return Response(status=status.HTTP_202_ACCEPTED)

    @action(detail=True, methods=['post'], permission_classes=[AllowAnyKindOfReviewer])
    def unsubscribe(self, request, **kwargs):
        addon = get_object_or_404(Addon, pk=kwargs['pk'])
        ReviewerSubscription.objects.filter(user=request.user, addon=addon).delete()
        return Response(status=status.HTTP_202_ACCEPTED)
...
class ReviewAddonVersionMixin(object):
    ...
    def get_queryset(self):
        ...
        qset = (
            Version.unfiltered.get_queryset()
            .only_translations()
            .filter(addon=addon)
            .order_by('-created')
        )
        ...
        # addon authors.
        can_view_unlisted = acl.check_unlisted_addons_reviewer(
            self.request
...
class ReviewAddonVersionDraftCommentViewSet(
    ...
    def get_extra_comment_data(self):
        return {'version': self.get_version_object().pk, 'user': self.request.user.pk}
