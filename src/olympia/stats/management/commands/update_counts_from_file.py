...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for group in groups:
            ...
            if get_date(filepath, sep) != day:
                raise CommandError('%s file contains data for another day' % filepath)
        ...
        for group, filepath in group_filepaths:
            ...
            for index, line in enumerate(count_file):
                ...
                if group == 'app':
                    day, addon_guid, app_id, app_ver, count, update_type = splitted
                else:
                ...
                if update_type in ['0', 'NULL', 'None', '', '\\N', '%UPDATE_TYPE%']:
                    update_type = None
                ...
                if update_type and update_type & 31 != 16:
                    log.debug("Update type doesn't add to 16: %s" % update_type)
                    continue
                ...
                if addon_guid and addon_guid in guids_to_addon:
                    ...
                    log.debug(
                        u"Addon {guid} doesn't exist.".format(guid=addon_guid.strip())
                    )
                    continue
    ...
    def update_version(self, update_count, version, count):
        ...
        update_count.versions = update_inc(update_count.versions, version, count)
    ...
    def update_status(self, update_count, status, count):
        ...
        if status in VALID_STATUSES:
            update_count.statuses = update_inc(update_count.statuses, status, count)
    ...
    def update_app(self, update_count, app_id, app_ver, count):
        ...
        if app_id not in VALID_APP_GUIDS or not re.match(APPVERSION_REGEX, app_ver):
            return
        ...
        update_count.applications.update({app_id: update_inc(app, app_ver, count)})
    ...
    def update_locale(self, update_count, locale, count):
        ...
        if re.match(LOCALE_REGEX, locale):
            update_count.locales = update_inc(update_count.locales, locale, count)
