...
class TestIndexStats(TestCase):
    ...
    def setUp(self):
        ...
        self.downloads = DownloadCount.objects.order_by('-date').values_list(
            'id', flat=True
        )
        self.updates = UpdateCount.objects.order_by('-date').values_list(
            'id', flat=True
        )
        self.theme_users = ThemeUserCount.objects.order_by(
            '-date'
    ...
    def test_by_date_range(self, tasks_mock):
        call_command('index_stats', addons=None, date='2009-06-01:2009-06-07')
    ...
    def test_index_latest(self):
        ...
        with mock.patch('olympia.stats.cron.call_command') as call:
            ...
            call.assert_called_with(
                'index_stats', addons=None, date='%s:%s' % (start, finish)
            )
    ...
    def test_addons_collections(self):
        ...
        for col_id in (80, collection2.pk):
            for dls, dt in vals:
                AddonCollectionCount.objects.create(
                    addon_id=3615, collection_id=col_id, count=dls, date=dt
                )
        ...
        assert (
            CollectionAddon.objects.get(
                addon_id=3615, collection_id=80
            ).downloads
            == 15
        )
