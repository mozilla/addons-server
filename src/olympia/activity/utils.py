    ...
    with translation.override('en-US'):
        if note.action == amo.LOG.REQUEST_INFORMATION.id:
            if addon.pending_info_request:
                ...
                if days_left > 9:
                    author_context_dict['number_of_days_left'] = '%d days' % days_left
                elif days_left > 1:
                    author_context_dict['number_of_days_left'] = '%s (%d) days' % (
                        apnumber(days_left),
                        days_left,
                    )
                else:
            ...
            reviewer_subject = u'Mozilla Add-ons: %s %s' % (addon.name, version.version)
        else:
