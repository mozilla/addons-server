...
class TestEventLog(EditorTest):
    ...
    def test_enddate_filter(self):
        ...
        assert r.status_code == 200
        assert pq(r.content)('tbody td').eq(0).text() == 'Jan 1, 2011 12:00:00 AM'
...
class TestReviewLog(EditorTest):
    ...
    def test_search_comment_exists(self):
        ...
        assert r.status_code == 200
        assert pq(r.content)('#log-listing tbody tr.hide').eq(0).text() == 'hello'
    ...
    def test_addon_missing(self):
        self.make_approvals()
        r = self.client.get(self.url)
        assert pq(r.content)('#log-listing tr td').eq(1).text() == 'Add-on has been deleted.'

    def test_request_info_logs(self):
        self.make_an_approval(amo.LOG.REQUEST_INFORMATION)
        r = self.client.get(self.url)
        assert pq(r.content)('#log-listing tr td a').eq(1).text() == 'needs more information'

    def test_super_review_logs(self):
        self.make_an_approval(amo.LOG.REQUEST_SUPER_REVIEW)
        r = self.client.get(self.url)
        assert pq(r.content)('#log-listing tr td a').eq(1).text() == 'needs super review'

    def test_comment_logs(self):
        self.make_an_approval(amo.LOG.COMMENT_VERSION)
        r = self.client.get(self.url)
        assert pq(r.content)('#log-listing tr td a').eq(1).text() == 'commented'
...
class TestHome(EditorTest):
    ...
    def test_deleted_review(self):
        ...
        assert doc('.row').eq(0).text().strip().split('.')[0] == 'editor deleted Review for yermom '
    ...
    def undelete_review(self, review, allowed):
        ...
        doc = pq(self.client.get(url).content)
        assert doc('#submit-undelete-review').attr('value') == 'Undelete' == allowed
    ...
    def test_stats_user_position_ranked(self):
        ...
        assert el.text() == "You're #1 with 2 reviews"
...
class TestQueueBasics(QueueTest):
    ...
    def test_paginator_when_many_pages(self):
        ...
        assert doc('.data-grid-bottom .num-results').text() == u'Results 1 \u2013 1 of 2'
    ...
    def test_navbar_queue_counts(self):
        ...
        assert r.status_code == 200
        doc = pq(r.content)
        assert doc('#navbar li.top ul').eq(0).text() == 'Fast Track (0) Full Reviews (2) Pending Updates (2) ' 'Preliminary Reviews (2) Moderated Reviews (0)'
...
class TestUnlistedQueueBasics(TestQueueBasics):
    ...
    def test_navbar_queue_counts(self):
        ...
        assert r.status_code == 200
        doc = pq(r.content)
        assert doc('#navbar li.top ul').eq(1).text() == 'Full Reviews (2) Pending Updates (2) Preliminary Reviews (2)'
...
class TestModeratedQueue(QueueTest):
    ...
    def test_skip_score(self):
        self.setup_actions(reviews.REVIEW_MODERATE_SKIP)
        assert ReviewerScore.objects.filter(note_key=amo.REVIEWED_ADDON_REVIEW) .count() == 0
    ...
    def test_remove(self):
        ...
        assert pq(r.content)('table .more-details').attr('href') == reverse('editors.eventlog.detail', args=[logs[0].id])
    ...
    def test_remove_score(self):
        self.setup_actions(reviews.REVIEW_MODERATE_DELETE)
        assert ReviewerScore.objects.filter(note_key=amo.REVIEWED_ADDON_REVIEW) .count() == 1
...
class TestPerformance(QueueTest):
    ...
    def test_performance_other_user_not_admin(self):
        ...
        r = self.client.get(self.get_url([10482]))
        doc = pq(r.content)
        assert doc('#select_user').length == 0  # Don't let them choose editors.
...
class TestQueueSearch(SearchTest):
    ...
    def test_search_by_addon_name_admin(self):
        ...
        assert sorted(self.named_addons(r)) == ['Admin Reviewed', 'Not Admin Reviewed']
    ...
    def test_search_by_many_addon_types(self):
        ...
        r = self.search(addon_type_ids=[amo.ADDON_THEME,
                                        amo.ADDON_SEARCH])
        assert sorted(self.named_addons(r)) == ['Justin Bieber Search Bar', 'Justin Bieber Theme']
    ...
    def test_preserve_single_platform_files(self):
        ...
        r = self.search(platform_ids=[amo.PLATFORM_WIN.id])
        doc = pq(r.content)
        assert doc('#addon-queue tbody td').eq(6).find('div').attr('title') == 'Windows'
    ...
    def test_search_by_version_requires_app(self):
        ...
        assert r.status_code == 200
        assert r.context['search_form'].errors.as_text() == '* max_version\n  * Select a valid choice. 3.6 is not ' 'one of the available choices.'
    ...
    def test_form(self):
        ...
        r = self.search()
        doc = pq(r.content)
        assert doc('#id_application_id').attr('data-url') == reverse('editors.application_versions_json')
        assert doc('#id_max_version option').text() == 'Select an application first'
        r = self.search(application_id=amo.MOBILE.id)
        doc = pq(r.content)
        assert doc('#id_max_version option').text() == ' '.join([av.version for av in AppVersion.objects.filter(application=amo.MOBILE.id)])
    ...
    def test_application_versions_json(self):
        ...
        assert data['choices'] == [[av, av] for av in [u''] + [av.version for av in AppVersion.objects.filter(application=amo.MOBILE.id)]]
...
class TestReview(ReviewBase):
    ...
    def test_info_comments_requested(self):
        ...
        assert response.context['form'].errors['comments'][0] == 'This field is required.'
    ...
    def test_page_title(self):
        ...
        assert doc('title').text() == '%s :: Editor Tools :: Add-ons for Firefox' % self.addon.name
    ...
    def test_admin_flagged_addon_actions_as_editor(self):
        ...
        assert response.context['form'].errors['action'] == [u'Select a valid choice. public is not one of the available ' u'choices.']
...
class TestReviewPreliminary(ReviewBase):
    ...
    def test_prelim_multiple_files(self):
        ...
        self.client.post(self.url, data)
        assert [amo.STATUS_DISABLED, amo.STATUS_LITE] == [f.status for f in self.version.files.all().order_by('status')]
...
class TestEditorMOTD(EditorTest):
    ...
    def test_form_errors(self):
        ...
        doc = pq(r.content)
        assert doc('#editor-motd .errorlist').text() == 'This field is required.'
...
class TestLeaderboard(EditorTest):
    ...
    def test_leaderboard_ranks(self):
        ...
        assert get_cells() == [users[2].display_name, users[1].display_name, amo.REVIEWED_LEVELS[0]['name'], users[0].display_name]

        self._award_points(users[0], 1)
        assert get_cells() == [users[2].display_name, users[1].display_name, users[0].display_name, amo.REVIEWED_LEVELS[0]['name']]

        self._award_points(users[0], -1)
        self._award_points(users[2], (amo.REVIEWED_LEVELS[1]['points'] -
                                      amo.REVIEWED_LEVELS[0]['points']))
        assert get_cells() == [users[2].display_name, amo.REVIEWED_LEVELS[1]['name'], users[1].display_name, amo.REVIEWED_LEVELS[0]['name'], users[0].display_name]
