...
class TestThemeEdit(amo.tests.TestCase):
    fixtures = ['base/users', 'base/licenses']

    def setUp(self):
        self.addon = addon_factory(type=amo.ADDON_PERSONA)
        self.addon.persona.license = License.objects.all()[0]
        self.addon.persona.save()
        AddonUser.objects.create(
            user=UserProfile.objects.get(username='regularuser'),
            addon=self.addon, role=amo.AUTHOR_ROLE_DEV)

        self.url = reverse('devhub.themes.edit', args=[self.addon.slug])
    ...
    def test_redirect(self):
        """/addon/:id => /addon/:id/edit"""
        r = self.client.get('/en-US/developers/theme/%s/' % self.addon.slug,
                            follow=True)
        self.assert3xx(r, self.url)

    def test_edit_basic(self):
        url = reverse('devhub.themes.section',
                      args=[self.addon.slug, 'basic', 'edit'])

        tags = ['tag3', 'tag2', 'tag1']
        for t in tags:
            Tag(tag_text=t).save_tag(self.addon)
        tags = [tag + 'new' for tag in tags]

        old_name = self.addon.name
        data = {'name': 'new name', 'slug': 'test_slug', 'summary': 'newsumry',
                'tags': ', '.join(tags)}
        ...
        self.addon.reload()
        eq_(unicode(self.addon.name), data['name'])
        eq_(self.addon.name.id, old_name.id)

        eq_(unicode(self.addon.slug), data['slug'])
        eq_(unicode(self.addon.summary), data['summary'])

        eq_([unicode(t) for t in self.addon.tags.all()], sorted(tags))

    def test_edit_license(self):
        url = reverse('devhub.themes.section',
                      args=[self.addon.slug, 'license', 'edit'])

        r = self.client.post(url, {'license': '9'})
        eq_(r.status_code, 200)
        # Can't reload since we care about persona is cached, not the addon.
        eq_(Addon.objects.get(id=self.addon.id).persona.license.builtin, 9)
