...
import amo
import amo.tests
from users.models import PersonaAuthor, UserProfile
...
class TestCommand(amo.tests.TestCase):
    ...
    def test_designers(self):
        addon = Addon.objects.create(type=amo.ADDON_PERSONA)
        persona = Persona.objects.create(persona_id=3, addon=addon)
        self.cmd.designers = [[3], [4]]
        self.cmd.handle(commit='yes')
        assert 'Not found persona 4 for user some_username' in self.cmd.logs
        user = UserProfile.objects.get(email='foo@bar.com')
        eq_(PersonaAuthor.objects
                         .filter(author=user, persona=persona).count(), 1)
