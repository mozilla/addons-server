...
class TestGetFeaturedIds(amo.tests.TestCase):
    ...
    def test_by_app(self):
        assert set(get_featured_ids(amo.FIREFOX)) == set(self.all_locales)

    def test_by_type(self):
        assert set(get_featured_ids(amo.FIREFOX, 'xx', 1)) == set(self.no_locale_type_one)

    def test_by_locale(self):
        assert set(get_featured_ids(amo.FIREFOX)) == set(self.all_locales)
        assert set(get_featured_ids(amo.FIREFOX, 'xx')) == set(self.no_locale)
        assert set(get_featured_ids(amo.FIREFOX, 'en-US')) == set(self.no_locale + self.en_us_locale)
...
class TestGetCreaturedIds(amo.tests.TestCase):
    ...
    def test_by_category(self):
        assert set(get_creatured_ids(self.category, None)) == set(self.no_locale)

    def test_by_locale(self):
        assert set(get_creatured_ids(self.category, 'en-US')) == set(self.no_locale + self.en_us_locale)
