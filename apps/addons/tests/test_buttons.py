...
class TestButtonHtml(ButtonTest):
    ...
    def test_basics(self):
        ...
        assert reverse('addons.versions', args=[a.id]) == install.attr('data-versions')
        assert 'addon name' == install.attr('data-name')
        assert install.attr('data-min') is None
        assert install.attr('data-max') is None
    ...
    def test_featured(self):
        self.addon.is_featured.return_value = True
        doc = self.render()
        assert ['install', 'featuredaddon'] == doc('.install').attr('class').split()
    ...
    def test_unreviewed_detailed_warning(self):
        ...
        assert warning.text() == 'This add-on has not been reviewed by Mozilla. Learn more'

    def test_lite_detailed_warning(self):
        self.addon.status = amo.STATUS_LITE
        self.version.is_lite = True
        warning = self.render(detailed=True)('.install-shell .warning')
        assert warning.text() == 'This add-on has been preliminarily reviewed by Mozilla.' ' Learn more'
