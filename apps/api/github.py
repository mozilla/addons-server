...
from apps.files.utils import SafeUnzip
...
class GithubCallback(object):
    ...
    def comment(self, data):
        print data
        self.post(self.data['comment_url'], data=data)


class GithubRequest(forms.Form):
    comment_url = forms.URLField(required=False)
    ...
    def get_comment(self):
        number = str(self.data['pull_request']['number'])
        return (
            self.repo['pulls_url']
            .replace('{/number}', '/' + number) + '/comments')
