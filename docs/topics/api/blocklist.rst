...
.. http:get:: /api/v4/blocklist/block/(int:block_id|string:guid)
    ...
    :>json string|object|null url: A url to the report/request that detailed why the add-on should potentially be blocked.  Typically a bug report on bugzilla.mozilla.org.  (See :ref:`Outgoing Links <api-overview-outgoing>`)
