...
1. **BuildKit**:
   ...
   - **Enabling BuildKit**: Buildkit is enabled by default on Docker Engine v23.0 and later.
   Ensure BuildKit is enabled by setting the environment variable `DOCKER_BUILDKIT=1`.
...
2. **Bake**:
   ...
   - **Using Bake**: We mostly use bake to enable building via docker-compose in a consistent way across local and CI builds.
   Note we use a `build` target in the `docker-compose.yml` file to define the build context and Dockerfile for the addons-server image.
...
4. **Rebuilding Images**:
   - Use `make up` to rebuild the Docker images if you make changes to the Dockerfile or dependencies:
   Remember, `make up` is idempotent so it will ensure your image is built, and running based on the latest changes.

This section provides a thorough understanding of the Dockerfile stages, build process using BuildKit and Bake, and commands to manage the Docker containers for the **addons-server** project. For more detailed information on specific commands, refer to the upcoming sections in this documentation.
